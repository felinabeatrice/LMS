      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            ...
            <div className="text-gray-500 text-sm mt-0.5">Completed Payments</div>
          </div>
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <div className="text-2xl font-bold text-amber-600">{payments.filter((p) => p.status === 'pending').length}</div>
            <div className="text-gray-500 text-sm mt-0.5">Pending Payments</div>
          </div>
        </div>
        {error && (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  {['ID', 'Student', 'Course', 'Amount', 'Status', 'Access', 'Date'].map(h => (
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.length === 0 ? (
                  ...
                  <tr key={p.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-4 text-xs text-gray-400">#{p.id}</td>
                    <td className="px-5 py-4">
                      <div className="font-medium text-gray-900 text-sm">{p.student?.name}</div>
                    ...
                    <td className="px-5 py-4">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${p.access_granted ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
