    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setStats({
          ...
          students: c.reduce((sum, x) => sum + (x._count?.enrollments || 0), 0),
        });
      } catch (err) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4 mb-8">
          {[
            ...
            { label: 'Students', value: stats?.students, icon: '👥', color: 'bg-purple-50' },
          ].map((s) => (
            <div key={s.label} className="bg-white rounded-xl border border-gray-200 p-4">
              <div className={`w-9 h-9 rounded-lg flex items-center justify-center text-lg mb-2 ${s.color}`}>
                {s.icon}
              </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          ...
          >
            ...
            <div>
              ...
              <div className="text-blue-100 text-xs mt-0.5">Build a new learning experience</div>
            </div>
          </Link>
          <Link
            to="/instructor/courses"
            className="bg-white hover:bg-gray-50 border border-gray-200 text-gray-800 rounded-xl p-5 flex items-center gap-3 transition-colors"
          >
            <span className="text-2xl">📋</span>
            <div>
              <div className="font-semibold">Manage Courses</div>
              <div className="text-gray-400 text-xs mt-0.5">Edit, update or delete courses</div>
            </div>
          </Link>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link to="/instructor/courses" className="text-sm text-blue-600 hover:text-blue-700 font-medium">
              View all
            </Link>
          </div>
          {courses.length === 0 ? (
            <div className="text-center py-8">
              ...
              <Link
                to="/instructor/courses/create"
                className="inline-block mt-3 bg-blue-600 text-white px-5 py-2 rounded-lg text-sm font-medium hover:bg-blue-700"
              >
                Create First Course
              </Link>
            </div>
                ...
                <div key={course.id} className="flex items-center gap-4 p-3 rounded-lg border border-gray-100 hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-400">
                      {course._count?.enrollments || 0} students — {course.category?.name}
                    </p>
                  </div>
