...
const MyEnrollments = () => {
  ...
  const [paying,      setPaying]      = useState(null); // paymentId being paid
  const [payMsg,      setPayMsg]      = useState({});   // { [courseId]: msg }
  ...
  const handlePay = async (courseId) => {
    ...
    try {
      // Find pending payment for this course
      ...
      if (!pending) {
        setPayMsg((m) => ({
          ...m,
          [courseId]: 'No pending payment found.',
        }));
        return;
      }
      ...
      setPayMsg((m) => ({
        ...m,
        [courseId]: '✅ Payment successful! Access granted.',
      }));
      fetchEnrollments(); // Refresh
    } catch (err) {
      setPayMsg((m) => ({
        ...m,
        [courseId]: err.response?.data?.message || 'Payment failed.',
      }));
    } finally {
      setPaying(null);
    }
              ...
              return (
                ...
                >
                  <div className="flex flex-col sm:flex-row">
                    ...
                    {/* Thumbnail */}
                    <div className="w-full sm:w-48 h-32 bg-gradient-to-br
