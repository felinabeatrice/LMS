    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { icon: '📚', label: 'Courses Enrolled', value: stats?.totalEnrolled ?? 0, color: 'bg-blue-50',   to: '/student/enrollments' },
            { icon: '✅', label: 'With Access',       value: stats?.withAccess    ?? 0, color: 'bg-green-50',  to: '/student/enrollments' },
            { icon: '💳', label: 'Payments Made',     value: stats?.totalPayments ?? 0, color: 'bg-purple-50', to: '/student/payments'    },
            { icon: '💰', label: 'Total Spent',       value: `$${stats?.totalSpent ?? '0.00'}`, color: 'bg-amber-50', to: '/student/payments' },
          ].map((s) => (
